...
def ttest_plot(side, namekey, data1, data2, groupnum = "1-6", threshold=0.05):
...
for namekey in namemap.keys():
    ttest_plot("RT",namekey, df.loc[df['group'] == 1,"RT"+ namekey], df.loc[df['group'] == 6,"RT"+namekey])
    ttest_plot("LT",namekey, df.loc[df['group'] == 1,"LT"+ namekey], df.loc[df['group'] == 6,"RT"+namekey])
